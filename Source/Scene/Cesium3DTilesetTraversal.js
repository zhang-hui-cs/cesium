    ...
    for (i = 0; i < length; ++i) {
        ...
        if (checkRefines) {
            ...
            if (!child._inRequestVolume) {
                ...
                childRefines = executeEmptyTraversal(
                    tileset,
                    child,
                    frameState
                );
...
function canTraverse(tileset, tile) {
    ...
    // FIXME: 自定义特定区域内 SSE 阈值
    if (tileset.specialVolume && tile._boundingVolume) {
        const intersect = tileset.specialVolume.cullingVolume.computeVisibility(
        ...
        if (intersect != Intersect.OUTSIDE) {
            return (
                tile._screenSpaceError >
                tileset.specialVolume.maximumScreenSpaceError
            );
        }
    }
...
function executeTraversal(
    ...
    while (stack.length > 0) {
        ...
        var baseTraversal = inBaseTraversal(
            tileset,
            tile,
            baseScreenSpaceError
        );
